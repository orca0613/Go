...
export function changePoint(point: number, operator: string) {
  ...
  fetch(`${API_URL}${USERDETAIL_DB_PATH}/change-point`, {
    ...
    body: JSON.stringify({name, point, operator})
  })
    .catch(error => console.error("Error: ", error))
}

export function changeCount(operator: string, name: string) {
  ...
  fetch(`${API_URL}${PROBLEMINFO_DB_PATH}/change-count`, {
    ...
    body: JSON.stringify({problemId, operator, name}),
  })
    .catch(error => console.error("Error: ", error))
}
...
export function deleteElement(where: string) {
  const idx = problemStore.getState().curIndex
  const problemId = problemStore.getState().problemList[idx]._id
  const name = localStorage.getItem(USERNAME)
  const token = localStorage.getItem(TOKEN)
