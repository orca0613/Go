...
import { useGetSampleLikedQuery } from "../../../slices/sampleProblemApiSlice"
import { LANGUAGE_IDX, LIKED, PAGE, SORTING_IDX, USERINFO, problemsPerPage } from "../../../util/constants"
...
export default function LikedProblem() {

  const userInfo: UserInfo = JSON.parse(sessionStorage.getItem(USERINFO) || initialUserInfo)
  const liked = JSON.stringify(userInfo.liked)

  const { data: problems, isLoading: gsLoading } = useGetSampleLikedQuery(liked)
  ...
  return (
    <Box textAlign="center">
      <Typography sx={{mt: 2}}>{menuWords.liked[languageIdx]} {problems? `(${problems.length})` : ""}</Typography>
      {divider}
