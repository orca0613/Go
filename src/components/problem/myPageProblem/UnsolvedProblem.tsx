...
import { useGetSampleUnsolvedQuery } from "../../../slices/sampleProblemApiSlice"
import { LANGUAGE_IDX, PAGE, SORTING_IDX, UNRESOLVED, USERINFO, problemsPerPage } from "../../../util/constants"
import { getLanguageIdx, getUnsolvedIdxArray, setProblemIndicies, sortingProblemList } from "../../../util/functions"
...
export default function UnsolvedProblem() {

  const userInfo: UserInfo = JSON.parse(sessionStorage.getItem(USERINFO) || initialUserInfo)
  const unsolved = JSON.stringify(getUnsolvedIdxArray(userInfo.tried, userInfo.solved))

  const { data: problems, isLoading: gsLoading } = useGetSampleUnsolvedQuery(unsolved)
  ...
  return (
    <Box textAlign="center">
      <Typography sx={{mt: 2}}>{menuWords.unresolved[languageIdx]} {problems? `(${problems.length})` : ""}</Typography>
      {divider}
