...
export default function CreatedProblem() {
  ...
  const created = JSON.stringify(userInfo.created)

  const { data: problems, isLoading: gsLoading } = useGetSampleCreatedQuery(created)
  ...
  return (
    <Box textAlign="center">
      <Typography sx={{mt: 2}}>{menuWords.createdProblem[languageIdx]} {problems? `(${problems.length})` : ""}</Typography>
      {divider}
