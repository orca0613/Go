...
import { useGetSampleSolvedQuery } from "../../../slices/sampleProblemApiSlice"
import { LANGUAGE_IDX, PAGE, SOLVED, SORTING_IDX, USERINFO, problemsPerPage } from "../../../util/constants"
...
export default function SolvedProblem() {
  ...
  const solved = JSON.stringify(userInfo.solved)

  const { data: problems, isLoading: gsLoading } = useGetSampleSolvedQuery(solved)
  ...
  return (
    <Box textAlign="center">
      <Typography sx={{mt: 2}}>{menuWords.solved[languageIdx]} {problems? `(${problems.length})` : ""}</Typography>
      {divider}
